...
class YOLODataset(BaseDataset):
    ...
    def get_image_and_label(self, index: int) -> dict[str, Any]:
        """Return a sample and optionally attach turbidity measured before random augmentation."""
        label = super().get_image_and_label(index)
        import os

        if os.getenv("TURBIDITY_PSEUDO_SOURCE", "preaugment").lower() == "preaugment":
            from ultralytics.data.turbidity import raw_turbidity

            image = torch.from_numpy(np.ascontiguousarray(label["img"].transpose(2, 0, 1))).unsqueeze(0)
            label["raw_t_preaugment"] = raw_turbidity(image).squeeze(0)
        return label
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "text_feats", "sem_masks", "raw_t_preaugment"}:
        ...
        # ==================== 修改开始：在 Batch 层级计算水体浑浊度伪标签 ====================
        import os
        import torch
        pseudo_source = os.getenv("TURBIDITY_PSEUDO_SOURCE", "preaugment").lower()
        if pseudo_source == "preaugment":
            # New experiment: the scalar was measured before RandomPerspective/LetterBox/padding.
            raw_t = new_batch.pop("raw_t_preaugment").view(-1)
        elif pseudo_source == "postaugment":
            # Original experiment path retained in full for reproducibility.
            imgs = new_batch["img"].float()
            B = imgs.shape[0]
            gray = imgs.mean(dim=1, keepdim=True)
            diff_y = gray[:, :, 1:, :] - gray[:, :, :-1, :]
            diff_x = gray[:, :, :, 1:] - gray[:, :, :, :-1]
            grad_var = diff_x.reshape(B, -1).var(dim=1) + diff_y.reshape(B, -1).var(dim=1)
            mean_brightness = gray.reshape(B, -1).mean(dim=1)
            raw_t = (mean_brightness + 1e-5) / (grad_var + 1e-5)
        else:
            raise ValueError(
                f"Unknown TURBIDITY_PSEUDO_SOURCE={pseudo_source!r}; expected preaugment or postaugment"
            )

        # 6. 归一化约束到 0~1 之间。默认使用仅由训练集标定的稳健分位数映射。
        # 旧公式完整保留为 legacy 模式，便于通过环境变量无损恢复旧实验：
        # TURBIDITY_PSEUDO_MODE=legacy python tools/run_at_yolov11_seeds.py ...
        pseudo_mode = os.getenv("TURBIDITY_PSEUDO_MODE", "soft").lower()
        if pseudo_mode == "legacy":
            # 原始实验公式：固定经验缩放常数 10.0。
            pseudo_t = torch.clip(raw_t / 10.0, 0.0, 1.0)
        elif pseudo_mode == "robust":
            from ultralytics.data.turbidity import normalize_turbidity

            pseudo_t = normalize_turbidity(raw_t)
        elif pseudo_mode == "soft":
            from ultralytics.data.turbidity import soft_normalize_turbidity

            pseudo_t = soft_normalize_turbidity(raw_t)
        else:
            raise ValueError(f"Unknown TURBIDITY_PSEUDO_MODE={pseudo_mode!r}; expected soft, robust, or legacy")

        # 7. 存入 batch 字典中，送往网络前向传播和 Loss 计算 (形状变更为 [B, 1])
        new_batch["pseudo_t"] = pseudo_t.unsqueeze(1)
        # ==================== 修改结束 ====================================================
