...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor | tuple): The last output of the model, or along with turbidity_t.
        """
        ...
        for m in self.model:
            ...
            if module_name == 'TABlock':
                # 将 T detach 后传入，切断检测 Loss 对浑浊度分支的梯度反传
                # 修改：加上 dtype=x.dtype
                t_in = turbidity_t.detach() if turbidity_t is not None else torch.zeros((x.size(0), 1), device=x.device,
                                                                                        dtype=x.dtype)
                x = m(x, t=t_in)
            else:
                x = m(x)  # 原有的 run 逻辑

            if module_name == 'TurbidityHead':
                turbidity_t = x  # 记录当前 batch 算出的浑浊度 T
            # ---------------- 修改结束 ----------------

            y.append(x if m.i in self.save else None)  # save output

            if visualize:
                from ultralytics.utils.plotting import feature_visualization  # 确保导入
        ...
        # ---------------- 新增：训练状态下返回 T ----------------
        # 只有在训练模式（self.training）并且成功提取到 T 时，才返回 tuple (x, turbidity_t)
        # 这样不会影响推理部署时直接输出检测框的逻辑
        if self.training and turbidity_t is not None:
            return x, turbidity_t
        # --------------------------------------------------------
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                if isinstance(output, tuple) and len(output) == 2 and isinstance(output[1], torch.Tensor):
                    output = output[0]  # 只保留前面的检测特征字典，丢弃 turbidity_t
                # if self.end2end:
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        #######
            ...
            args = [*args[1:]]
        ########
        elif m in {TABlock, TurbidityHead}:
            args = [ch[f], *args]  # 自动获取上一层的真实通道数作为 c1
        #######
        else:
            c2 = ch[f]
