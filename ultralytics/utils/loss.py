...
class v8DetectionLoss:
    ...
    def __call__(
        ...
        # 2. 拦截并分离数据：
        # 我们在第二步的 _predict_once 中返回了 (x, turbidity_t)
        # 所以这里的 preds 会是一个长度为 2 的元组，且第二个元素是我们算出来的浑浊度 Tensor
        if isinstance(preds, tuple) and len(preds) == 2 and isinstance(preds[1], torch.Tensor):
            preds, pred_t = preds  # 将检测输出 (preds) 和浑浊度输出 (pred_t) 分开

        # 3. 将干净的 preds 丢给原版 YOLO 去算基础检测 Loss
        total_loss, loss_items = self.loss(self.parse_output(preds), batch)

        # 4. 计算并加入我们的浑浊度 Loss
        if pred_t is not None and 'pseudo_t' in batch:
            import torch.nn as nn
            # 从 batch 中获取 DataLoader 传过来的伪标签，并对齐设备和维度
            target_t = batch['pseudo_t'].to(pred_t.device).view(-1, 1).float()

            # 使用均方误差计算回归损失
            loss_t = nn.MSELoss()(pred_t, target_t)

            # Turbidity supervision strength. The original experiment used 0.001;
            # set TURBIDITY_LOSS_WEIGHT=0.001 to reproduce it without changing code.
            import os

            lambda_t = float(os.getenv("TURBIDITY_LOSS_WEIGHT", "0.1"))

            # 累加到总 Loss (乘以 batch size 是为了和 YOLO 官方 loss_items 保持统一量级)
            batch_size = batch['img'].shape[0]
            total_loss = total_loss + lambda_t * loss_t * batch_size

            # (可选) 为了让你在终端训练打印时能看到 loss_t，把它拼接到 loss_items 的末尾
            #loss_items = torch.cat((loss_items, loss_t.detach().unsqueeze(0)))
